...
# TODO: Maybe we can do another decorator here...
...
class LoggerFile(CustomLogger):
    ...
    def logger_handler(self, logging_object):
        ...
        logger_handler setter.
        """
        handler = logging_object.FileHandler(self.wrapped.output_file)
        handler.setLevel(self.wrapped.logging_level)
        formatter = logging_object.Formatter(self.inner_parameters.logging_file_format)
        handler.setFormatter(formatter)
        self._logger_handler = handler
        return self._logger_handler
